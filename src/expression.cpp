...
map_boost global_hash_table_2;

expression::expression(simple_type t, int var) : hash_table(global_hash_table_2) ,t(t), var(var)  {}
...
struct expression *operator+(const struct expression & e1, const struct expression & e2) {
	...
	if (e1.t == _INT) {
		if (e2.t == _INT) {
			...
			ret->code << e1.code.str() << e2.code.str() << "%x" << conversion << " = sitofp i32 %x" << e1.var << " to double\n";
			ret->code << "%x" << ret->getVar() << " = fadd double %x" << e2.var << ", %x" << conversion << "\n";
		}
	}
		...
		if (e2.t == _INT) {
			int conversion = new_var();
			ret = new expression(_DOUBLE, new_var(), e1.hash_table);
			ret->code << e1.code.str() << e2.code.str() << "%x" << conversion << " = sitofp i32 %x" << e2.var << " to double\n";
			ret->code << "%x" << ret->getVar() << " = fadd double %x" << e1.var << ", %x" << conversion << "\n";
		}
		else if (e2.t == _DOUBLE) {
			ret = new expression(_DOUBLE, new_var(), e1.hash_table);
			ret->code << e1.code.str() << e2.code.str() << "%x" << ret->getVar() << " = fadd double %x" << e1.var << ", %x" << e2.var << "\n";
		}
	}
	return ret;
}
...
struct expression *operator-(const struct expression & e1, const struct expression & e2) {
	...
	if (e1.t == _INT) {
		if (e2.t == _INT) {
			ret = new expression(_INT, new_var(), e1.hash_table);
			ret->code << e1.code.str() << e2.code.str() << "%x" << ret->getVar() << " = sub i32 %x" << e1.var << ", %x" << e2.var << "\n";
		}
		else if (e2.t == _DOUBLE) {
			int conversion = new_var();
			ret = new expression(_DOUBLE, new_var(), e1.hash_table);
			ret->code << e1.code.str() << e2.code.str() << "%x" << conversion << " = sitofp i32 %x" << e1.var << " to double\n";
			ret->code << "%x" << ret->getVar() << " = fsub double %x" << e2.var << ", %x" << conversion << "\n";
		}
	}
	else if(e1.t == _DOUBLE) {
		if (e2.t == _INT) {
			...
			ret = new expression(_DOUBLE, new_var(), e1.hash_table);
			ret->code << e1.code.str() << e2.code.str() << "%x" << conversion << " = sitofp i32 %x" << e2.var << " to double\n";
			ret->code << "%x" << ret->getVar() << " = fsub double %x" << e1.var << ", %x" << conversion << "\n";
		}
		else if (e2.t == _DOUBLE) {
			ret = new expression(_DOUBLE, new_var(), e1.hash_table);
			ret->code << e1.code.str() << e2.code.str() << "%x" << ret->getVar() << " = fsub double %x" << e1.var << ", %x" << e2.var << "\n";
		}
	}
	return ret;
}
...
struct expression *operator*(const struct expression & e1, const struct expression & e2) {
	...
	if (e1.t == _INT) {
		if (e2.t == _INT) {
			ret = new expression(_INT, new_var(), e1.hash_table);
			ret->code << e1.code.str() << e2.code.str() << "%x" << ret->getVar() << " = mul i32 %x" << e1.var << ", %x" << e2.var << "\n";
		}
		else if (e2.t == _DOUBLE) {
			int conversion = new_var();
			ret = new expression(_DOUBLE, new_var(), e1.hash_table);
			ret->code << e1.code.str() << e2.code.str() << "%x" << conversion << " = sitofp i32 %x" << e1.var << " to double\n";
			ret->code << "%x" << ret->getVar() << " = fmul double %x" << e2.var << ", %x" << conversion << "\n";
		}
	}
	else if(e1.t == _DOUBLE) {
		if (e2.t == _INT) {
			...
			ret = new expression(_DOUBLE, new_var(), e1.hash_table);
			ret->code << e1.code.str() << e2.code.str() << "%x" << conversion << " = sitofp i32 %x" << e2.var << " to double\n";
			ret->code << "%x" << ret->getVar() << " = fmul double %x" << e1.var << ", %x" << conversion << "\n";
		}
		else if (e2.t == _DOUBLE) {
			ret = new expression(_DOUBLE, new_var(), e1.hash_table);
			ret->code << e1.code.str() << e2.code.str() << "%x" << ret->getVar() << " = fmul double %x" << e1.var << ", %x" << e2.var << "\n";
		}
	}
	return ret;
}
...
struct expression *operator/(const struct expression & e1, const struct expression & e2) {
	...
	if (e1.t == _INT) {
		if (e2.t == _INT) {
			ret = new expression(_INT, new_var(), e1.hash_table);
			ret->code << e1.code.str() << e2.code.str() << "%x" << ret->getVar() << " = sdiv i32 %x" << e1.var << ", %x" << e2.var << "\n";
		}
		else if (e2.t == _DOUBLE) {
			int conversion = new_var();
			ret = new expression(_DOUBLE, new_var(), e1.hash_table);
			ret->code << e1.code.str() << e2.code.str() << "%x" << conversion << " = sitofp i32 %x" << e1.var << " to double\n";
			ret->code << "%x" << ret->getVar() << " = fdiv double %x" << e2.var << ", %x" << conversion << "\n";
		}
	}
	else if(e1.t == _DOUBLE) {
		if (e2.t == _INT) {
			...
			ret = new expression(_DOUBLE, new_var(), e1.hash_table);
			ret->code << e1.code.str() << e2.code.str() << "%x" << conversion << " = sitofp i32 %x" << e2.var << " to double\n";
			ret->code << "%x" << ret->getVar() << " = fdiv double %x" << e1.var << ", %x" << conversion << "\n";
		}
		else if (e2.t == _DOUBLE) {
			ret = new expression(_DOUBLE, new_var(), e1.hash_table);
			ret->code << e1.code.str() << e2.code.str() << "%x" << ret->getVar() << " = fdiv double %x" << e1.var << ", %x" << e2.var << "\n";
		}
	}
	return ret;
}
...
struct expression *operator%(const struct expression & e1, const struct expression & e2) {
	...
	if (e1.t == _INT) {
		if (e2.t == _INT) {
			ret = new expression(_INT, new_var(), e1.hash_table);
			ret->code << e1.code.str() << e2.code.str() << "%x" << ret->getVar() << " = srem i32 %x" << e1.var << ", %x" << e2.var << "\n";
		}
		else if (e2.t == _DOUBLE) {
			int conversion = new_var();
			ret = new expression(_DOUBLE, new_var(), e1.hash_table);
			ret->code << e1.code.str() << e2.code.str() << "%x" << conversion << " = sitofp i32 %x" << e1.var << " to double\n";
			ret->code << "%x" << ret->getVar() << " = frem double %x" << e2.var << ", %x" << conversion << "\n";
		}
	}
	else if(e1.t == _DOUBLE) {
		if (e2.t == _INT) {
			...
			ret = new expression(_DOUBLE, new_var(), e1.hash_table);
			ret->code << e1.code.str() << e2.code.str() << "%x" << conversion << " = sitofp i32 %x" << e2.var << " to double\n";
			ret->code << "%x" << ret->getVar() << " = frem double %x" << e1.var << ", %x" << conversion << "\n";
		}
		else if (e2.t == _DOUBLE) {
			ret = new expression(_DOUBLE, new_var(), e1.hash_table);
			ret->code << e1.code.str() << e2.code.str() << "%x" << ret->getVar() << " = frem double %x" << e1.var << ", %x" << e2.var << "\n";
		}
	}
	return ret;
}
...
struct expression *operator==(const struct expression & e1, const struct expression & e2) {
	...
	case _INT:
		...
		case _DOUBLE:
			...
			ret->code << "%x" << ret->getVar() << " = fcmp oeq double %x" << conversion << ", %x" << e2.var << "\n";
			break;
	...
	case _DOUBLE:
		...
		case _INT:
			...
			ret->code << "%x" << ret->getVar() << " = fcmp oeq double %x" << e1.var << ", %x" << conversion << "\n";
			break;

		case _DOUBLE:
			ret = new expression(_BOOL, new_var(), e1.hash_table);
			ret->code << e1.code.str() << e2.code.str();
			ret->code << "%x" << ret->getVar() << " = fcmp oeq double %x" << e1.var << ", %x" << e2.var << "\n";
			break;
...
struct expression *operator!=(const struct expression & e1, const struct expression & e2) {
	...
	case _INT:
		...
		case _DOUBLE:
			...
			ret->code << e1.code.str() << e2.code.str();
			ret->code << "%x" << conversion << " = sitofp i32 %x" << e1.var << " to double\n";
			ret->code << "%x" << ret->getVar() << " = fcmp one double %x" << conversion << ", %x" << e2.var << "\n";
			break;
	...
	case _DOUBLE:
		...
		case _INT:
			...
			ret->code << e1.code.str() << e2.code.str();
			ret->code << "%x" << conversion << " = sitofp i32 %x" << e2.var << " to double\n";
			ret->code << "%x" << ret->getVar() << " = fcmp one double %x" << e1.var << ", %x" << conversion << "\n";
			break;

		case _DOUBLE:
			ret = new expression(_BOOL, new_var(), e1.hash_table);
			ret->code << e1.code.str() << e2.code.str();
			ret->code << "%x" << ret->getVar() << " = fcmp one double %x" << e1.var << ", %x" << e2.var << "\n";
			break;
...
struct expression *operator<(const struct expression & e1, const struct expression & e2) {
	...
	case _INT:
		...
		case _DOUBLE:
			...
			ret->code << e1.code.str() << e2.code.str();
			ret->code << "%x" << conversion << " = sitofp i32 %x" << e1.var << " to double\n";
			ret->code << "%x" << ret->getVar() << " = fcmp olt double %x" << conversion << ", %x" << e2.var << "\n";
			break;
	...
	case _DOUBLE:
		...
		case _INT:
			...
			ret->code << e1.code.str() << e2.code.str();
			ret->code << "%x" << conversion << " = sitofp i32 %x" << e2.var << " to double\n";
			ret->code << "%x" << ret->getVar() << " = fcmp olt double %x" << e1.var << ", %x" << conversion << "\n";
			break;

		case _DOUBLE:
			ret = new expression(_BOOL, new_var(), e1.hash_table);
			ret->code << e1.code.str() << e2.code.str();
			ret->code << "%x" << ret->getVar() << " = fcmp olt double %x" << e1.var << ", %x" << e2.var << "\n";
			break;
...
struct expression *operator>(const struct expression & e1, const struct expression & e2) {
	...
	case _INT:
		...
		case _DOUBLE:
			...
			ret->code << e1.code.str() << e2.code.str();
			ret->code << "%x" << conversion << " = sitofp i32 %x" << e1.var << " to double\n";
			ret->code << "%x" << ret->getVar() << " = fcmp ogt double %x" << conversion << ", %x" << e2.var << "\n";
			break;
	...
	case _DOUBLE:
		...
		case _INT:
			...
			ret->code << e1.code.str() << e2.code.str();
			ret->code << "%x" << conversion << " = sitofp i32 %x" << e2.var << " to double\n";
			ret->code << "%x" << ret->getVar() << " = fcmp ogt double %x" << e1.var << ", %x" << conversion << "\n";
			break;

		case _DOUBLE:
			ret = new expression(_BOOL, new_var(), e1.hash_table);
			ret->code << e1.code.str() << e2.code.str();
			ret->code << "%x" << ret->getVar() << " = fcmp ogt double %x" << e1.var << ", %x" << e2.var << "\n";
			break;
...
struct expression *operator<=(const struct expression & e1, const struct expression & e2) {
	...
	case _INT:
		...
		case _DOUBLE:
			...
			ret->code << e1.code.str() << e2.code.str();
			ret->code << "%x" << conversion << " = sitofp i32 %x" << e1.var << " to double\n";
			ret->code << "%x" << ret->getVar() << " = fcmp ole double %x" << conversion << ", %x" << e2.var << "\n";
			break;
	...
	case _DOUBLE:
		...
		case _INT:
			...
			ret->code << e1.code.str() << e2.code.str();
			ret->code << "%x" << conversion << " = sitofp i32 %x" << e2.var << " to double\n";
			ret->code << "%x" << ret->getVar() << " = fcmp ole double %x" << e1.var << ", %x" << conversion << "\n";
			break;

		case _DOUBLE:
			ret = new expression(_BOOL, new_var(), e1.hash_table);
			ret->code << e1.code.str() << e2.code.str();
			ret->code << "%x" << ret->getVar() << " = fcmp ole double %x" << e1.var << ", %x" << e2.var << "\n";
			break;
...
struct expression *operator>=(const struct expression & e1, const struct expression & e2) {
	...
	case _INT:
		...
		case _DOUBLE:
			...
			ret->code << e1.code.str() << e2.code.str();
			ret->code << "%x" << conversion << " = sitofp i32 %x" << e1.var << " to double\n";
			ret->code << "%x" << ret->getVar() << " = fcmp oge double %x" << conversion << ", %x" << e2.var << "\n";
			break;
	...
	case _DOUBLE:
		...
		case _INT:
			...
			ret->code << e1.code.str() << e2.code.str();
			ret->code << "%x" << conversion << " = sitofp i32 %x" << e2.var << " to double\n";
			ret->code << "%x" << ret->getVar() << " = fcmp oge double %x" << e1.var << ", %x" << conversion << "\n";
			break;

		case _DOUBLE:
			ret = new expression(_BOOL, new_var(), e1.hash_table);
			ret->code << e1.code.str() << e2.code.str();
			ret->code << "%x" << ret->getVar() << " = fcmp oge double %x" << e1.var << ", %x" << e2.var << "\n";
			break;
